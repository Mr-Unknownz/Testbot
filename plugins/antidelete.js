...
/**
 * Full Anti-Delete + ViewOnce Recovery for Baileys lib/msg
 */
...
function saveDB() {
  ...
  } catch (e) { console.error('ANTIDELETE saveDB error:', e); }
}
...
function loadDB() {
  try {
    ...
    try { fs.writeFileSync(DB_FILE, '[]'); } catch (_) {}
  }
}
loadDB();

/* ---------- Helpers ---------- */
async function saveMediaMessageWrapper(messageObj, key) {
  try {
    const type = Object.keys(messageObj)[0];
    const filename = key?.id || Date.now();
    const buffer = await downloadMediaMessage({ type, message: messageObj }, filename);
    if (!buffer) return null;

    const ext = '.bin';
    const filePath = path.join(BASE_DIR, `${filename}${ext}`);
...
async function resendSavedMessage(conn, saved, sendTo) {
  try {
    ...
    if (!saved.media || !saved.media.filePath || !fs.existsSync(saved.media.filePath))
      return await conn.sendMessage(sendTo, { text: captionPrefix + (saved.text || '[deleted media]') });
  ...
  } catch (e) { console.error('ANTIDELETE resendSavedMessage error:', e); }
}

/* ---------- Core init ---------- */
function initAntiDelete(conn, opts = {}) {
    ...
    try {
      ...
      for (const msg of messages) {
        try {
          ...
          else if (msg.message?.viewOnceMessage) type = 'view_once';

          const saved = { key, pushName, from, timestamp, type, text, media: null };

          if (type !== 'text') saved.media = await saveMediaMessageWrapper(msg.message, key);

          const mapKey = key?.id || `${from}_${timestamp}`;
          savedMessages.set(mapKey, saved);
          saveDB();
        } catch (e) { console.error('ANTIDELETE inner save error:', e); }
      }
    } catch (e) { console.error('ANTIDELETE messages.upsert handler error:', e); }
  });
  ...
  conn.ev.on('messages.update', async (updates) => {
    try {
      ...
      for (const u of arr) {
        ...
        if (proto && typeof proto === 'object') {
          ...
          const mapKey = delKey?.id || `${delKey?.remoteJid}_${delKey?.fromMe ? 'self' : 'other'}`;
        ...
        if (u?.messageStubType === 68) {
          const mapKey = u.key?.id || `${u.key?.remoteJid}_${u.key?.fromMe ? 'self' : 'other'}`;
          const saved = savedMessages.get(mapKey);
          if (saved) {
            const target = (config.ANTI_DEL_PATH === 'inbox') ? (conn.user?.jid || saved.from) : saved.from;
            await resendSavedMessage(conn, saved, target);
    ...
    } catch (e) { console.error('ANTIDELETE messages.update handler error:', e); }
  });
