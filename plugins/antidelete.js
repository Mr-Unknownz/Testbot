...
/**
 * Full Anti-Delete plugin for Baileys-based bots (lib/msg version)
 */

const fs = require('fs');
const path = require('path');
const config = require('../settings/settings.json');
const { downloadMediaMessage } = require('../lib/msg'); // <<< use your lib/msg downloadMediaMessage
  ...
  try {
    ...
    if (!Array.isArray(arr)) {
      if (typeof arr === 'object' && arr !== null) {
        const converted = Object.entries(arr);
...
async function saveMediaMessageWrapper(conn, messageObj, key) {
  try {
    // Use lib/msg downloadMediaMessage directly
    const filename = key?.id || Date.now();
    const buffer = await downloadMediaMessage({ type: Object.keys(messageObj)[0], message: messageObj }, filename);
    if (!buffer) return null;

    const ext = '.bin'; // fallback extension, lib/msg can handle proper extension inside
    const filePath = path.join(BASE_DIR, `${filename}${ext}`);
    fs.writeFileSync(filePath, buffer);

    return { filePath, filename, mimetype: '' }; // mime not used here
  } catch (e) {
...
async function resendSavedMessage(conn, saved, sendTo) {
  try {
    ...
    if (!saved.media || !saved.media.filePath || !fs.existsSync(saved.media.filePath))
      return await conn.sendMessage(sendTo, { text: captionPrefix + (saved.text || '[deleted message - media missing]') });
